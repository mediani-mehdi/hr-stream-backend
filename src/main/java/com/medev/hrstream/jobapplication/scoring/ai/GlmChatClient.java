...
public class GlmChatClient implements AiChatClient {
    ...
    public GlmChatClient(
            @Qualifier("glmChatModel") ObjectProvider<OpenAiChatModel> modelProvider,
            AiProviderProperties props) {
        this.modelProvider = modelProvider;
        this.cfg = props.getProviders().getGlm();
    }
    ...
    @Override
    @CircuitBreaker(name = "ai-glm")
    public AiChatResult complete(String prompt) {
        OpenAiChatModel model = modelProvider.getIfAvailable();
        if (model == null) {
            throw new ProviderTransientException(NAME, "no GLM chat model configured", null);
        }
        try {
            ...
            throw new ProviderTransientException(NAME, "glm call failed", e);
        }
    }
}
